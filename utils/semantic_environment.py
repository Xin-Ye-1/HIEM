...
class Semantic_Environment():
    def __init__(self,
        ...
        self.all_targets = json.load(open('%s/targets_info_all.json' % self.dir, 'r')).keys()
        class2id = json.load(open(os.path.join(cfg['codeDir'], 'Environment', 'class2id.json'), 'r'))
        self.all_targets_id = [int(class2id[t]) for t in self.all_targets]
    ...
    def get_visible_objects(self, state=None):
        ...
        if isinstance(state, tuple):
            (x, y, orien) = state
            state = self.map[(x, y)] * 4 + orien
        return [t for t in self.all_bboxes[state].keys() if t in self.all_targets_id]
...
if __name__ == '__main__':
    # pass
    env = '5cf0e1e9493994e483e985c436b9d3bc'
    env = Semantic_Environment(env)
    features = env.get_state_feature(0)
