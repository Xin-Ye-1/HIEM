...
class Worker():
    def __init__(self,
        ...
        self.lowlevel_network_main, self.lowlevel_network_ex_main, self.lowlevel_network_ex_target = lowlevel_networks
        ...
        with sess.as_default():
            if FLAGS.load_model:
                ...
                if testing or FLAGS.continuing_training:
                    ...
                    sess.run(tf.global_variables_initializer())
                    ckpt = tf.train.get_checkpoint_state(FLAGS.pretrained_model_path)
                    # variable_to_restore = slim.get_variables_to_restore(include=['lowlevel/global/in/main'])
                    variable_to_restore = slim.get_variables_to_restore(exclude=['global_episodes', 'global_frames'])
                    variable_to_restore = [val for val in variable_to_restore if 'ex' not in val.name]
                    temp_saver = tf.train.Saver(variable_to_restore)
                    # var_list = {}
                    # for var in variable_to_restore:
                    #     var_list[var.name.replace('lowlevel/global/in/main', 'global/main').split(':')[0]] = var
    ...
    def _train_highlevel(self,
        ...
        with sess.as_default():
            ...
            next_q_prime = sess.run(self.highlevel_network_target.qvalue,
            ...
            next_q = sess.run(self.highlevel_network_main.qvalue,
            ...
            target_qvalues = batch[:, 7] + (1 - batch[:, 9]) * FLAGS.gamma * \
                             next_q_prime[range(FLAGS.batch_size), subtarget_id]

            highlevel_lr = self._get_learning_rate(FLAGS.highlevel_lr)

            qvalue_loss, _ \
                = sess.run([self.highlevel_network_main.qvalue_loss,
                           ...
                           feed_dict={self.highlevel_network_main.visions:np.stack(batch[:, 0]),
                                      ...
                                      self.highlevel_network_main.target_qvalue:target_qvalues,
    ...
    def _train_ex_lowlevel(self,
                         sess):
        # replay_buffer:
        # [0:vision, 1:depth, 2:target_input, 3:action, 4:er  5:done,
        #  6:new_vision, 7:new_depth]
        with sess.as_default():
            batch = self.lowlevel_replay_buffer.sample(FLAGS.batch_size)
            next_q_prime = sess.run(self.lowlevel_network_ex_target.action_qvalues,
                                    feed_dict={self.lowlevel_network_ex_target.visions: np.stack(batch[:, 6]),
                                               self.lowlevel_network_ex_target.depths: np.stack(batch[:, 7]),
                                               self.lowlevel_network_ex_target.targets: np.stack(batch[:, 2])})
            next_q = sess.run(self.lowlevel_network_ex_main.action_qvalues,
                              feed_dict={self.lowlevel_network_ex_main.visions: np.stack(batch[:, 6]),
                                         self.lowlevel_network_ex_main.depths: np.stack(batch[:, 7]),
                                         self.lowlevel_network_ex_main.targets: np.stack(batch[:, 2])})
            ...
            qvalue_loss, _ \
                = sess.run([self.lowlevel_network_ex_main.action_qvalue_loss,
                            self.lowlevel_network_ex_main.highlevel_update],
                           feed_dict={self.lowlevel_network_ex_main.visions: np.stack(batch[:, 0]),
                                      self.lowlevel_network_ex_main.depths: np.stack(batch[:, 1]),
                                      self.lowlevel_network_ex_main.targets: np.stack(batch[:, 2]),
                                      self.lowlevel_network_ex_main.chosen_actions: batch[:, 3],
                                      self.lowlevel_network_ex_main.target_action_qvalues: target_qvalues,
                                      self.lowlevel_network_ex_main.highlevel_lr: highlevel_lr})
            return qvalue_loss
    ...
    def _run_training_episode(self,
        ...
        if testing:
            ...
            alpha = 0#max((10000 - max(self.episode_count - 0, 0)) / float(10000),0)
        ...
        if np.random.rand() < alpha:
            ...
            for _ in range(FLAGS.max_episode_steps):
                ...
                term_prob = sess.run(self.highlevel_network_main.terminations,
                                     feed_dict={
                                         self.highlevel_network_main.visions: [np.vstack(vision_feature)],
                ...
                if highlevel_steps == 0 or sub_done or termination or lowlevel_steps == FLAGS.max_lowlevel_episode_steps:
                    ...
                    if np.random.rand() < ep_end:
                        ...
                        qvalues = sess.run(self.highlevel_network_main.qvalue,
                ...
                if not sub_done:
                    if np.random.rand() < ep_end or subtarget_id == FLAGS.num_labels-1:
                        ...
                        lowlevel_qvalues = sess.run(self.lowlevel_network_main.qvalues,
                                                    feed_dict={self.lowlevel_network_main.visions:[np.vstack(vision_feature)],
                                                               self.lowlevel_network_main.depths:[np.vstack(depth_feature)],
                                                               self.lowlevel_network_main.subtargets:[subtarget_input]})
                        action = np.argmax(lowlevel_qvalues[0])
                ...
                if not testing:
                    if episode_steps < FLAGS.max_episode_steps - 1:
                        # self.highlevel_replay_buffer.add(np.reshape(np.array([np.vstack(vision_feature),
                    ...
                    if self.episode_count > FLAGS.replay_start_size and \
                            len(self.lowlevel_replay_buffer.buffer) >= FLAGS.batch_size and \
                            self.frame_count % FLAGS.highlevel_update_freq == 0:
                        ex_loss = self._train_ex_lowlevel(sess=sess)
                        ex_losses.append(ex_loss)
            ...
            for _ in range(FLAGS.max_episode_steps):
                ...
                if np.random.rand() < epsilon:
                    ...
                    qvalues = sess.run(self.lowlevel_network_ex_main.action_qvalues,
                                       feed_dict={self.lowlevel_network_ex_main.visions: [np.vstack(vision_feature)],
                                                  self.lowlevel_network_ex_main.depths: [np.vstack(depth_feature)],
                                                  self.lowlevel_network_ex_main.targets: [target_input]})
                ...
                subtargets_buffer.append(int(class2id[target]))
                actions_buffer.append(action)
