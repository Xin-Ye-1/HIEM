...
class Worker():
    ...
    def _train_highlevel(self,
        ...
        with sess.as_default():
            ...
            next_terminations = 1#next_terminations[range(FLAGS.batch_size), batch[:, 4].astype('int32')]
                                                       ...
                                                       feed_dict={self.highlevel_network_target.visions: np.stack(
                                                           batch[:, 10]),
                                                                  ...
                                                                  self.highlevel_network_target.targets: np.stack(
                                                                      batch[:, 3])})
            next_terminations = 1#next_terminations[range(batch_size), batch[:, 4].astype('int32')]
    ...
    def _run_training_episode(self,
        ...
        if testing:
            ...
            alpha = 1 #max((10000 - max(self.episode_count - 100, 0)) / float(10000),0)
